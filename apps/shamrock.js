...
export class shamrock extends plugin {
  ...
  async version (e) {
    ...
    try {
      ...
      let client = new ShamrockRepoClient(Cfg.Adapter.Shamrock.githubKey)
  ...
  async apk (e) {
    ...
    if (!e.msg.includes('测试')) {
      ...
      // beta
      if (!Cfg.Adapter.Shamrock.githubKey) {
      ...
      if (fs.existsSync(dest)) {
        ...
        // todo move to client
        try {
          filePath = await common.downloadFile(url, 'shamrock/beta/' + name + '.zip', { Authorization: `bearer ${Cfg.Adapter.Shamrock.githubKey}` })
        } catch (err) {
          console.error(err)
          await e.reply('安装包下载错误：' + err.message)
          return
