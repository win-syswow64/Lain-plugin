...
class LLOneBotCore {
  ...
  async notice (data) {
    ...
    switch (data.notice_type) {
      ...
      case 'group_card': {
        ...
        let gml = await Bot[this.id].gml.get(data.group_id)
        let user = await gml.get(data.user_id)
  ...
  async LoadBot () {
    ...
    Bot[this.id] = {
      ...
      sendLike: async (user_id, times) => await this.sendLike(user_id, times),
  ...
  async ICQQEvent (data) {
    ...
    const noticePostType = async function () {
      if (e.sub_type === 'poke') {
        e.action = e.poke_detail?.action || '戳一戳';
 ...
 */
      ...
      switch (i.type) {
        ...
        case 'image':
          try {
            ...
            message.push({ type: 'image', data: { file, summary: await this.getImgSummary() } })
          } catch (err) {
            message.push({ type: 'text', data: { text: JSON.stringify(err) } })
            raw_message.push(JSON.stringify(err))
          }
  ...
  /**
   * 
   * @returns {Promise<string>} - 返回一言
   */
  async getImgSummary() {
    const baseUrl = 'https://hitokoto.fuxuan.org';
    const bakUrl = 'https://v1.hitokoto.cn';
    ...
    try { 
      ...
      data = await fetch(bakUrl)
        .then(res => res.json())
        .then(data => data?.hitokoto)
        .catch(err => { throw err })
    }

    if (data) { return data; } else { return '[动画表情]'; }
  }
  ...
  async sendLike (user_id, times = 1) {
    const userId = user_id?.toString() || user_id;
    return await api.send_like(this.id, userId, times)
  }
