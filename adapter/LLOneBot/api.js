...
let api = {
  ...
  */
  ...
  async httpApi (id, action, headers, data, query = '') {
    if (!Cfg.Adapter.Shamrock.baseUrl || !Cfg.Adapter.Shamrock.baseUrl.startsWith('http')) {
    ...
    let baseUrl = Cfg.Adapter.Shamrock.baseUrl
    let token = Cfg.Adapter.Shamrock.token
    if (token) {
  ...
  */
   ...
   * 点赞
   * @param {string} uin - 机器人QQ 通过e.bot、Bot调用无需传入
   * @param {string} user_id - 目标QQ
   * @param {number} times - 点赞次数
   */
  async send_like (uin, user_id, times = 1) {
    const params = { user_id, times }
    return await this.SendApi(uin, 'send_like', params)
  },
