...
export default class MilkyAdapter {
  id = "Milky"
  name = "Milky"
  version = "1.0.0"
  path = this.name

  constructor (config = Cfg.Milky) {
    this.config = config || {}
    this.load()
  }
  ...
  load () {
      ...
      let { host, port, prefix = "", connection = "ws" } = this.config
      this.defaultTimeout = (this.config.http_timeout || 15) * 1000
      ...
      if (connection === "ws") {
        const wsUrl = cleanHost.includes(":")
          ? `${wsProtocol}://${cleanHost}${prefix}/event${this.config.access_token ? `?access_token=${this.config.access_token}` : ""}`
          : `${wsProtocol}://${cleanHost}:${port}${prefix}/event${this.config.access_token ? `?access_token=${this.config.access_token}` : ""}`
    ...
    connectWs(apiBaseUrl, wsUrl) {
      const heartbeatInterval = (this.config.ws?.heartbeat || 30) * 1000
      const reconnectInterval = (this.config.ws?.reconnect_interval || 10) * 1000
    ...
    setupWebhook(apiBaseUrl) {
      const path = this.config.webhook?.path || "/milky"
    ...
    async onConnect(ws, apiBaseUrl) {
      ...
      try {
        const loginInfo = await this.callApi(apiBaseUrl, this.config.access_token, "get_login_info")
        ...
        if (exists) {
          ...
          Bot[self_id].sendApi = (action, params) =>
            this.callApi(apiBaseUrl, this.config.access_token, action, params)
          ...
          if (!Bot.adapter.includes(self_id)) Bot.adapter.push(self_id)
        }

        this.attachBotMethods(self_id, apiBaseUrl, this.config.access_token)
    ...
    attachBotMethods(self_id, apiBaseUrl, token) {
      ...
      Object.assign(bot, {
        ...
        getGroupList: () => bot.gl,
        getGuildList: () => bot.guilds,
        getMemberMap: group_id => this.getMemberMap(ctx({ group_id })),
        readMsg: async () => common.recvMsg(self_id, 'Milky', true),
        MsgTotal: async type => common.MsgTotal(self_id, 'Milky', type, true),
...
common.info('Lain-plugin', 'Milky适配器加载完成')
