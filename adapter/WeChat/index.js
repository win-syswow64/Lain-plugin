...
class AdapterComWeChat {
  ...
  async message(data) {
    ...
    switch (data.detail_type) {
      ...
      case 'private':
        /** 转置消息后给喵崽 */
        return await Bot.emit('message', await this.ICQQMessage(this.id, data))
      /** 群聊消息事件 */
      case 'group':
        /** 转置消息后给喵崽 */
        return await Bot.emit('message', await this.ICQQMessage(this.id, data))
  ...
  async request(data) {
    ...
    switch (data.detail_type) {
      ...
      case 'wx.friend_request':
        if (Cfg.Adapter.ComWeChat.autoFriend) {
  ...
  async notice(data) {
    ...
    switch (data.detail_type) {
      ...
      case 'wx.get_private_poke':
        /** 转置消息后给喵崽 */
        return await Bot.emit('notice', await this.ICQQMessage(this.id, data))
      /** 群聊拍一拍 */
  ...
  async LoadBot() {
    ...
    Bot[this.id] = {
      ...
      gml: new Map(),
      guilds: new Map(),
      nickname: Cfg.Adapter.ComWeChat.name || data.user_name,
      adapter: 'ComWeChat',
      uin: this.id,
      tiny_id: this.id,
      ...
      pickUser: (user_id) => this.pickFriend(this.id, user_id),
      ...
      makeForwardMsg: async (message) => await this.makeForwardMsg(this.id, message),
      getMsg: (msg_id) => '',
  ...
  async ICQQMessage(id, data) {
    ...
    if (detail_type === 'private' || detail_type === 'wx.get_private_poke') {
      data.friend = {
        recallMsg: () => '暂未支持',
        makeForwardMsg: async (forwardMsg) => await this.makeForwardMsg(id, forwardMsg),
        getChatHistory: () => '暂未支持',
  ...
  pickGroup(id, group_id) {
    return {
      ...
      recallMsg: async () => '',
      /** 制作转发 */
      makeForwardMsg: async (message) => await this.makeForwardMsg(id, message),
